...
export class EventCalendarComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    MeetingList.forEach((item: any) => {
      ...
      if (
        ...
        (this.endDate ? currDate < this.endDate : true)
      ) {
        switch (item.type) {
          case 'project':
            this.meetings.push(
    ...
    this.meetings = this.meetings.sort((a: MeetingModel, b: MeetingModel) =>
      a.date > b.date ? 1 : -1
    );
  ...
  public filterMeetings(filterType: string) {
    ...
    if (filterType === 'all') {
      ...
      this.showingMeetings = this.meetings.filter(
        (currMeeting: MeetingModel | CEMeetingModel | ProjectModel) => {
